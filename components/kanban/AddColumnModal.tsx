...
export function AddColumnModal({ open, projectId, onClose, onSuccess }: AddColumnModalProps) {
    ...
    const createMutation = useMutation({
        ...
        onSuccess: (newCol) => {
            ...
            toast.success(t('columns.addedSuccess', { name: newCol.name, defaultValue: `Column ${newCol.name} added` }))
            onSuccess?.(newCol)
            onClose()
            setName("")
        },
        onError: (error: any) => {
            if (error?.response?.status === 409) {
                toast.error(t('columns.nameExists', { defaultValue: "Column name already exists" }))
            } else {
                toast.error(t('columns.addError', { defaultValue: "Unable to add column" }))
            }
        },
    })
        ...
        <Dialog open={open} onOpenChange={val => !val && onClose()}>
            <DialogContent className="sm:max-w-[420px] p-0 overflow-hidden rounded-2xl border-none shadow-2xl">
                <DialogHeader className="px-6 pt-6 pb-0">
                    <DialogTitle className="text-xl font-bold text-gray-900">{t('columns.addNew', { defaultValue: "Add New Column" })}</DialogTitle>
                </DialogHeader>

                <div className="p-6 space-y-6">
                    {/* Preview Section */}
                    <div className="space-y-2">
                        <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wider">{t('common.preview', { defaultValue: "Preview" })}</label>
                        <div
                        ...
                        >
                            {name || t('columns.namePlaceholder', { defaultValue: "Column Name" })}
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        <label className="text-[11px] font-bold text-gray-500 uppercase tracking-wider flex justify-between">
                            <span>{t('columns.name', { defaultValue: "Column Name" })}</span>
                        ...
                        <input
                            ...
                            placeholder={t('columns.nameInputPlaceholder', { defaultValue: "e.g. In Testing, QA..." })}
                    ...
                    {/* Color Picker */}
                    <div className="space-y-2">
                        <label className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">{t('columns.color', { defaultValue: "Color" })}</label>
                ...
                <div className="p-4 bg-gray-50/50 border-t border-gray-100 flex gap-3">
                    ...
                    >
                        {createMutation.isPending ? t('common.processing', { defaultValue: "Processing..." }) : t('common.create', { defaultValue: "Create Column" })}
                    </button>
                </div>
            </DialogContent>
        </Dialog>
