...
export default function ResetPasswordForm() {
    ...
    async function onResetSubmit(data: any) {
        try {
            setIsLoading(true);
            const res = await AuthService.resetPassword({ email, otp: data.otp, newPassword: data.newPassword, confirmPassword: data.confirmPassword });
    ...
    return (
        <div className="w-full">
            ...
            <div className="mb-8">
                ...
                <p className="text-gray-400 text-sm leading-relaxed">
                    {step === 1
                        ...
                        : <span>Code sent to <span className="text-blue-600 font-semibold">{email}</span>. Enter it below.</span>
                    }
                </p>
            </div>
            ...
            {step === 1 ? (
                <Form {...emailForm}>
                    <form onSubmit={emailForm.handleSubmit(onEmailSubmit)} className="space-y-5" autoComplete="off">
                        <FormField
                        ...
                        <div className="text-center pt-2">
                            <Link href="/signin" className="inline-flex items-center text-sm text-gray-400 font-medium hover:text-gray-700 transition-colors group">
                ...
                <Form {...resetForm}>
                    <form onSubmit={resetForm.handleSubmit(onResetSubmit)} className="space-y-4" autoComplete="off">
                            ...
                            render={({ field }) => (
                                <FormItem className="space-y-1.5">
                                    <div className="flex justify-between items-center">
                                        <Label className="text-gray-600 text-xs font-semibold uppercase tracking-wide">Verification Code</Label>
                                        <span className={`text-xs font-semibold ${countdown > 0 ? 'text-blue-500' : 'text-red-400'}`}>
                                            {countdown > 0 ? `Valid for ${formatTime(countdown)}` : "Expired"}
                                        </span>
                                    </div>
                                    <FormControl>
                                        <div className="relative">
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem className="space-y-1.5">
                                    <Label className="text-gray-600 text-xs font-semibold uppercase tracking-wide">New Password</Label>
                                    <FormControl>
                                        <div className="relative">
                        ...
                        <div className="text-center">
                            <button type="button" onClick={() => setStep(1)} className="text-xs text-gray-400 hover:text-gray-700 transition-colors">
                                Wrong email? Go back
                            </button>
                        </div>
                    </form>
