...
type LoginFieldErrors = Record<string, string>;
...
function LoginFormContent() {
  ...
  const mutation = useMutation<LoginMutationResponse, unknown, LoginFormValues>(
    {
        ...
        if (res.data) {
          ...
          if (authData.userId || authData.email) {
            setUser({
              ...
              password: "",
              displayName: authData.fullName ?? authData.email ?? "",
      ...
      onError: (error) => {
        ...
        if (status === 401) {
          ...
          const fieldErrors = getBeFieldErrors(
            error,
          ) as LoginFieldErrors | null;
          if (fieldErrors) {
