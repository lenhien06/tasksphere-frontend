...
export class AuthService {
    ...
    static async login(userData: LoginFormValues): Promise<ApiResponse<AuthResponse>> {
        const response = await apiJava.post<ApiResponse<AuthResponse>>(`${this.PREFIX}/signin`, userData)
        return response.data
    }

    static async register(userData: RegisterFormValues): Promise<ApiResponse<AuthResponse>> {
        const signupRequest = {
    ...
    static async loginNext(userData: LoginFormValues): Promise<ApiResponse<AuthResponse>> {
        const response = await apiNext.post<ApiResponse<AuthResponse>>(`${this.PREFIX}/signin`, userData)
        return response.data
    }

    static async registerNext(userData: RegisterFormValues): Promise<ApiResponse<AuthResponse>> {
        const response = await apiNext.post<ApiResponse<AuthResponse>>(`${this.PREFIX}/signup`, userData)
        return response.data
    }

    static async sendOtpNext(email: string): Promise<ApiResponse<void>> {
        const response = await apiNext.post<ApiResponse<void>>(`${this.PREFIX}/send-otp?email=${encodeURIComponent(email)}`);
        return response.data;
    }
    ...
    static async sendOtp(email: string): Promise<ApiResponse<void>> {
        const response = await apiJava.post<ApiResponse<void>>(`${this.PREFIX}/send-otp?email=${encodeURIComponent(email)}`);
        return response.data;
    }
